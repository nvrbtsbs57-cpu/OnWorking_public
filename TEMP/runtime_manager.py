...
# Racine projet BOT_GODMODE/BOT_GODMODE
...
class RuntimeWalletManager:
    ...
    - reçoit les événements runtime (tick, trade fermé),
    - maintient un snapshot JSON "dashboard-friendly" dans
      data/godmode/wallets_runtime.json,
    - expose les métriques globales attendues par le RiskEngine
      (WalletMetricsIface).
    """
    ...
    def from_config(
        ...
        *,
        ...
        # Snapshot initial dès l'init (si engine OK)
        try:
            mgr._write_snapshot()
        except Exception:
            log.exception(
    ...
    # ------------------------------------------------------------------
    # Accès au WalletFlowsEngine interne
    # ------------------------------------------------------------------
    ...
    def refresh_balances(self) -> None:
        """
        Hook appelé au début de chaque tick.
    ...
    def get_total_equity_usd(self) -> Decimal:
        ...
        if isinstance(wallets, dict):
            for w in wallets.values():
                # w peut être dict ou float (format legacy)
                if isinstance(w, dict):
                    bal = w.get("balance_usd")
                    total += _safe_decimal(bal)
                else:
                    total += _safe_decimal(w)
        else:
    ...
    # ------------------------------------------------------------------
    # Construction snapshot wallets_runtime.json
    # ------------------------------------------------------------------

    def _build_snapshot_from_engine(self) -> Dict[str, Any]:
        """
        Snapshot complet à partir du WalletFlowsEngine.

        IMPORTANT : ici on met bien, pour chaque wallet, un DICT
        (balance_usd, realized_pnl_today_usd, etc.) et PAS un float,
        afin d'être compatible avec test_live150_finance.print_snapshot().
        """
        ...
        for wid, state in states.items():
            ...
            wallets[wid] = {
                ...
                "consecutive_losing_trades": int(state.consecutive_losing_trades),
    ...
    def debug_snapshot(self) -> Dict[str, Any]:
        """
        Construit et retourne un snapshot complet pour les tests (M10 / LIVE_150).

        C'est cette méthode que `test_live150_finance.py` utilise :
        elle renvoie donc un dict avec `wallets` = {wallet_id: { ... }}.
        """
    ...
    def to_runtime_json(self) -> Dict[str, Any]:
        """
        Alias pour _build_snapshot, pour les tests/BO qui veulent directement
        le JSON runtime (compat éventuelle avec anciens appels).
        """
