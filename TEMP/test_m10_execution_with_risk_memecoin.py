...
from bot.trading.execution_with_risk import build_execution_with_risk_from_config

log = get_logger("test_m10_execution_with_risk_memecoin")
...
def build_execution_with_risk(
    ...
    IMPORTANT : on garde exactement les mêmes règles/tuyaux que le live,
    mais on reste en DRY_RUN (aucune TX envoyée).
    """
...
def log_wallets_snapshot_in_memory(runtime_wallet_manager: RuntimeWalletManager) -> None:
    """
    Snapshot in-memory via RuntimeWalletManager.to_runtime_json().

    On ne dépend pas du fichier wallets_runtime.json ici,
    on prend l'état direct du moteur M10.
    """
    try:
        snap = runtime_wallet_manager.to_runtime_json()
    except Exception as exc:  # pragma: no cover
        log.warning(
            "Impossible de récupérer le snapshot in-memory des wallets: %s", exc
        )
        return
...
def main(wait_seconds: float = 2.0) -> None:
    ...
    log.info("=== M10 – test_m10_execution_with_risk_memecoin (PAPER_ONCHAIN) ===")
    ...
    # 3) Snapshot wallets in-memory
    log_wallets_snapshot_in_memory(runtime_wallet_manager)

    # 4) Laisser le temps au snapshot writer de créer/mettre à jour execution_runtime.json.
    #    (En pratique, le writer écrit déjà au __init__, mais on garde un petit wait pour être safe.)
    log.info(
        "Attente de %.1fs pour laisser ExecutionRuntimeSnapshotWriter écrire execution_runtime.json...",
        wait_seconds,
    )
    time.sleep(wait_seconds)

    # 5) Lecture du snapshot d'exécution depuis le fichier JSON
    log_execution_snapshot_from_file()

    log.info("Test M10 ExecutionWithRisk terminé.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test M10: ExecutionWithRisk (PAPER_ONCHAIN) câblé sur RuntimeWalletManager LIVE_150."
    )
    parser.add_argument(
        "--wait",
        type=float,
        default=2.0,
        help="Temps d'attente avant lecture du snapshot execution_runtime.json (défaut: 2.0s).",
    )
