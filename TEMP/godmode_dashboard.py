...
def get_execution_runtime() -> Dict[str, Any]:
    """
    Expose l'état risk/execution écrit par M10 dans execution_runtime.json.

    L'API ne décide rien : elle renvoie le snapshot brut du moteur,
    plus un peu de méta pour le dashboard / les scripts M11.
    """
    finance_cfg = _get_finance_cfg()
    profile_id = finance_cfg.get("profile_id", "UNKNOWN")
    ...
    # Fallback propre si le fichier n'est pas encore écrit
    ...
    return {
        ...
        "updated_at": exec_runtime.get("updated_at"),
    }
