...
# Racine du projet: C:\Users\ME\Documents\BOT_GODMODE
...
def main() -> None:
    ...
    logger = logging.getLogger("wallets_runtime_snapshot")
    ...
    # Ça instancie RuntimeWalletManager -> écrit wallets_runtime.json
    wm = RuntimeWalletManager.from_config(
        raw_cfg,
        logger=logger.getChild("RuntimeWalletManager"),
    )

    snap = wm.debug_snapshot()
    logger.info("Wallets   : %d", len(snap))
    logger.info("Equity USD: %s", wm.get_total_equity_usd())
