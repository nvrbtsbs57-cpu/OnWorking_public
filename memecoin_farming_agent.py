...
class StubRandomMemecoinProvider:
    ...
    def scan_candidates(
        ...
        for _ in range(n):
            ...
            max_base = float(cfg.max_notional_usd) if cfg.max_notional_usd > 0 else max(
                min_notional * 2,
                1.0,
            )
...
class MemecoinStrategyEngine:
    """
    Moteur de stratégie memecoin (ENTRY/EXIT time-based).
    """
    ...
    def _candidate_to_signal(
        ...
        if key in self._open_positions:
            # On a déjà une position ouverte sur ce wallet/symbol.
            self._logger.debug(
        ...
        # Clamp notionnel (reste en Decimal)
        ...
        # ⚠️ notional_usd est un Decimal ici (compat avec PaperTrader via Decimal)
        signal = TradeSignal(
            id=sig_id,
        ...
        # Compat avec PaperTrader : certains moteurs lisent signal.chain / signal.entry_price
        try:
            signal.chain = candidate.chain  # type: ignore[attr-defined]
        except Exception:
            self._logger.debug(
                "Impossible de fixer attribute 'chain' sur TradeSignal",
                exc_info=True,
            )

        try:
            # Prix stub pour le mode dry_run : 1.0 si rien en meta
            signal.entry_price = float(candidate.meta.get("entry_price", 1.0))  # type: ignore[attr-defined]
        except Exception:
            self._logger.debug(
                "Impossible de fixer attribute 'entry_price' sur TradeSignal",
                exc_info=True,
            )
    ...
    def _generate_exit_signals(self) -> List[TradeSignal]:
        ...
        for key, pos in self._open_positions.items():
            ...
            # ⚠️ notional_usd reste en Decimal ici aussi
            ...
            # Compat PaperTrader : chain / entry_price sur EXIT aussi
            try:
                exit_sig.chain = pos.chain  # type: ignore[attr-defined]
            except Exception:
                self._logger.debug(
                    "Impossible de fixer attribute 'chain' sur EXIT TradeSignal",
                    exc_info=True,
                )

            try:
                # Stub : on garde le même prix "1.0" si rien d'autre
                exit_sig.entry_price = float(  # type: ignore[attr-defined]
                    meta.get("exit_price", 1.0)
                )
            except Exception:
                self._logger.debug(
                    "Impossible de fixer attribute 'entry_price' sur EXIT TradeSignal",
                    exc_info=True,
                )
...
def build_memecoin_strategy_from_config(
    ...
    provider: Optional[MemecoinDataProvider] = None
    if provider_kind in ("stub_random", "onchain_dry_run"):
        # Pour 'onchain_dry_run', on utilise aussi le stub aléatoire
        ...
        log.info(
            ...
            "(kind=%s, max_candidates_per_tick=%d, seed=%r)",
            provider_kind,
            max_cands,
            seed,
        )
